...
from Sh6Project import BOTLOG_CHATID, LOGGER
from Sh6Project.helpers.basic import edit_or_reply
from Sh6Project.helpers.misc import HAPP
...
async def shutdown_bot(client: Client, message: Message):
    if BOTLOG_CHATID:
        await client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**PyroSupra-Userbot** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(message, "**PyroSupra-Userbot Berhasil di matikan!**")
    if HAPP is not None:
...
async def logs_ubot(client: Client, message: Message):
    ...
    Sh6 = await edit_or_reply(message, "**Sedang Mengambil Logs Heroku**")
    with open("Logs-Heroku.txt", "w") as log:
        log.write(HAPP.get_log())
    await client.send_document(
        message.chat.id,
        "Logs-Heroku.txt",
        thumb="Sh6Project/resources/logo.jpg",
