...
from Sh6Project.helpers.tools import get_arg, get_text, resize_media
from Sh6Project.utils.tools import add_text_img, bash
...
async def kang(client: Client, message: Message):
    ...
    Sh6 = await edit_or_reply(message, "`Boleh juga ni stickernya colong ahh...`")
    media_ = None
    emoji_ = None
    is_anim = False
    is_video = False
    ...
    if replied and replied.media:
        if replied.photo:
            ...
            if not replied.sticker.file_name:
                await Sh6.edit("**Stiker tidak memiliki Nama!**")
                return
            ...
            await Sh6.edit("**File Tidak Didukung**")
            return
        media_ = await client.download_media(replied, file_name="ProjectMan/resources/")
    else:
        await Sh6.edit("**Silahkan Reply ke Media Foto/GIF/Sticker!**")
        return
    if media_:
        ...
        if exist is not False:
            try:
                ...
                return await Sh6.edit(f"**ERROR:** `{e}`")
...
async def tinying(client: Client, message: Message):
    ...
    if ik.endswith(".tgs"):
        ...
        await bash("lottie_convert.py json.json Sh6.tgs")
        file = "man.tgs"
        os.remove("json.json")
